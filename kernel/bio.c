...
struct {
  ...
  struct buf head[BUCKETSZ];
  struct spinlock lock[BUCKETSZ];
} bcache;
...
bget(uint dev, uint blockno)
{
  ...
  for (int i = 0; i < BUCKETSZ; i++)
  {
    int swbucket = (bucket + i) % BUCKETSZ;
    if (swbucket == bucket)
      continue;
    
    acquire(&bcache.lock[swbucket]);
    for (b = bcache.head[swbucket].prev; b != &bcache.head[swbucket]; b = b->prev)
    {
      if (b->refcnt == 0)
      {
        b->next->prev = b->prev;
        b->prev->next = b->next;
        release(&bcache.lock[swbucket]);
...
brelse(struct buf *b)
{
  ...
  int bucket = bucketnum(b->blockno);
  acquire(&bcache.lock[bucket]);
  b->refcnt--;
  if (b->refcnt == 0) {
